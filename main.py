...
mediapipeHand = MediapipeHand() #楊采璇 part1
gestureDetection=HandGestureDetection() #林祐靖 part2
quickDraw = QuickDraw() #林子云 part4
###
...
while cap.isOpened(): #每一楨影像都可以視為一張照片 一次while迴圈 就是處理一楨影像(一張照片)
    ...
    if mediapipeHand.HaveHandNow == 1 : #畫面中是否有手
        ...
        if(result=='1'):#食指->開始畫畫模式
            drawing=True
        elif (result=='5' and drawing==True):#結束畫畫模式
            #crop
            ...
            quickDraw.draw_detect(paintWindow)
            #clear&reload
            drawing=False
            positions_x=[] 
            positions_y=[] 
